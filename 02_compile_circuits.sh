...
circom src/withdraw.circom --r1cs --wasm --json -o build/
...
snarkjs zkey export solidityverifier build/withdraw.zkey build/verifier_withdraw.sol

cp build/verifier_withdraw.sol ./verifier/

cp build/withdraw_js/withdraw.wasm ./build_for_ui/
cp build/withdraw.zkey ./build_for_ui/